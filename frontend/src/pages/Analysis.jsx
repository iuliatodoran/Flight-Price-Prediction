...
// Modelele au fost antrenate pe zboruri cu days_left intre 1 si 49 (Clean_Dataset.csv);
// in afara acestui interval, predictiile extrapoleaza si devin nerealiste.
const MIN_DAYS_LEFT = 1;
const MAX_DAYS_LEFT = 49;
...
const MIN_FLIGHT_DATE = dateOffset(MIN_DAYS_LEFT);
const MAX_FLIGHT_DATE = dateOffset(MAX_DAYS_LEFT);
...
export default function Analysis() {
  ...
  const incomingFlight = location.state?.flight || null;
  ...
  useEffect(() => {
    if (incomingFlight && options && !autoRequested.current) {
  ...
  return (
    <div className="shell">
      <Nav active="analysis" />
      <header className="topbar" style={{ paddingTop: "16px" }}>
        <div>
          <h1 style={{ margin: 0 }}>Analiză ML</h1>
              ...
              <div className="form">
                ...
                <div className="row">
                  <div className="field">
                    ...
                    <input type="number" step="0.1" min="0.83" max="49.83" value={form.duration} onChange={(e) => setField("duration", e.target.value)} />
                  </div>
                  <div className="field">
                    ...
                    <span style={{ fontSize: "11px", color: "var(--muted)", marginTop: "4px", display: "block" }}>
                      Modelele au fost antrenate doar pe zboruri rezervate cu 1-49 zile înainte; date în afara acestui interval pot produce predicții nerealiste.
                    </span>
                  </div>
                </div>
              </div>
